...
class EventCorrelator:
    ...
    def _analyze_event_impact(self, stock_data, event_date, lookback_days, lookahead_days):
        ...
        try:
            ...
            # Calculate returns
            before_return = (before_data['Close'].iloc[-1] / before_data['Close'].iloc[0] - 1) * 100
            after_return = (after_data['Close'].iloc[-1] / after_data['Close'].iloc[0] - 1) * 100
            event_day_return = (stock_data['Close'].iloc[event_idx] / stock_data['Close'].iloc[event_idx - 1] - 1) * 100 if event_idx > 0 else 0

            # Calculate volatility
            before_volatility = before_data['Close'].pct_change().std() * np.sqrt(252) * 100
            after_volatility = after_data['Close'].pct_change().std() * np.sqrt(252) * 100

            return {
                'actual_event_date': actual_event_date.strftime('%Y-%m-%d'),
                ...
                'price_at_event': stock_data['Close'].iloc[event_idx]
            }
        ...
        for ticker, data in stock_data_dict.items():
            ...
            for date, return_pct in unusual_dates.items():
                movement_info = {
                    ...
                    'z_score': (return_pct - mean_return) / std_return,
                    'price': data.loc[date, 'Close'],
                    'volume': data.loc[date, 'Volume'] if 'Volume' in data.columns else None
                }
            ...
            for movement in movements:
                if not movement['nearby_events']:  # No nearby events
                    summary['unexplained_movements'].append({
                        'ticker': ticker,
                        'date': movement['date'],
                        'return_pct': movement['return_pct'],
